...
module.exports = function(onMessage, remoteBus, localBus) {
  return _createClient(onMessage, remoteBus, localBus);
};
...
function _createClient(onMessage, remoteBus, localBus) {
  ...
  function _request(operation) {
    ...
    if (operation.resp !== false) {
      stream.pipe(through(function(data, next) {
        remoteBus(mesh.op("data", { resp: operation.req, data: data }));
        next();
      }, function() {
        remoteBus(mesh.op("end", { resp: operation.req }));
      })).on("error", function(err) {
        remoteBus(mesh.op("error", { resp: operation.req, data: { message: err.message } }));
      });
    }
